...
class MultiPathPP(nn.Module):
    ...
    def forward(self, data, num_steps):
        ...
        assert_fn(torch.isfinite(target_mcg_input_data_linear).all())
        ...
        assert_fn(torch.isfinite(target_agents_embeddings).all())
        other_mcg_input_data_linear = self._interaction_mcg_linear(
            data["other/history/mcg_input_data"]
        )
        assert_fn(torch.isfinite(other_mcg_input_data_linear).all())
        ...
        assert_fn(torch.isfinite(roadgraph_mcg_embedding).all())
        ...
        assert_fn(torch.isfinite(covariance_matrices).all())
